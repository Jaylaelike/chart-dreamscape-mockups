...
const AdditionStationsContent = () => {
  ...
  const loading = isStationDataLoading || isDailyReporterDataLoading || isIrdHarmonicDataLoading || isTxControlDataLoading;
  ...
  const peaOnStations = dailyReporterData.filter(
    (station) =>
      station.PEA === "PEA : ON" || station.PEA === "PEA Electrical is OK"
  );
  const peaOffStations = dailyReporterData.filter(
    (station) =>
      station.PEA === "PEA : OFF" ||
      station.PEA === "PEA Electrical is a Proplem"
  );
  ...
  const peaStatusByFacility = dailyReporterData.reduce((acc, station) => {
    ...
    if (station.PEA === "PEA : ON" || station.PEA === "PEA Electrical is OK") {
      acc[facility].on++;
    } else if (
      station.PEA === "PEA : OFF" ||
      station.PEA === "PEA Electrical is a Proplem"
